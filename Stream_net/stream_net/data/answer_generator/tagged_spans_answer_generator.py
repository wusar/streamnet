...
class Tagged_Spans_AnswerGenerator(AnswerGenerator):
    """
    used for answering multi-span questions
    """
    ...
    def generate_answer(self, raw_data, instance: Dict[str, Any],update_instance=True) -> None:
        ...
        if len(spans) == 0:
            ...
            answer_tag = [0 for i in range(self.max_seq_length)]
            for span in spans:
                for i in range(span[0], span[1]+1):
                    answer_tag[i] = 1
            if update_instance:
                instance['answer_as_tagged_span'] = answer_tag
            return True
